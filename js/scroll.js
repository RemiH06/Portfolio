...
async function loadProjects() {
    ...
    projects.forEach((project, index) => {
        ...
        panel.innerHTML = `
            ...
            <p><strong>Author(s):</strong> ${project.autor}</p>
            <p><strong>Description:</strong> ${project.descripcion}</p>
            <p><strong>Utility:</strong> ${project.utilidad}</p>
            <p><strong>Langs & Tools:</strong> ${project.lenguajes.join(', ')} / ${project.herramientas.join(', ')}</p>
            <div class="imagenes-proyecto">
                ${project.imagenes.map(img => `<img src="${img}" alt="Project img">`).join('')}
            </div>
